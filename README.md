...
[[Preprint]][casa-arxiv] [[Models on Hugging Face]](https://huggingface.co/collections/kyutai/casa)

**CASA** is a vision-language fusion paradigm that aims to improve on  cross-attention while preserving its practical benefits. At its core, CASA fuses visual information through cross-attention, while additionally enabling text-to-text interaction inside the fusion layers, inside local attention windows. This simple modification to enables natural gating and substantially improves performance of cross-attention, substantially closing the gap to standard token insertion methods.

 This repository contains inference code for pretrained CASA models and we are  planning to release training code in the future.
 
Alongside the code, we release several CASA variants with 2B to 3B parameters, which can be found in the following [HuggingFace collection](https://huggingface.co/collections/kyutai/casa). Below you can find example code to run the models, evaluate them and use them for live captioning of videos.

For more technical details on CASA, see our [blog post][blog] and [preprint][casa-arxiv].

## CASA in a nutshell

<p align="center">
  <img src="assets/casa_model.png" alt="CASA model overview" width="80%">
</p>

<table align="center">
  <tr>
    <td align="center" width="45%">
      <img src="assets/sa_layer.png" alt="Standard self-attention layer" width="100%">
      <br>
      <em><strong>(i)</strong> Standard self-attention layer</em>
    </td>
    <td align="center" width="45%">
      <img src="assets/casa_layer.png" alt="CASA layer" width="100%">
      <br>
      <em><strong>(ii)</strong> CASA layer w/ asymmetric block-wise attention</em>
    </td>
  </tr>
</table>
...
- [**Qwen2.5-VL-3B**](https://huggingface.co/Qwen/Qwen2.5-VL-3B-Instruct), a **pretrained VLM** which originally handles visual inputs by directly adding image tokens to its token stream. In this setting we keep the backbone VL<> frozen and adapt it to CASA by training only the additional CASA layers.
...
> 🔹 **CASA models**  
> We release **`kyutai/CASA-Helium1-VL-2B`** and **`kyutai/CASA-Qwen-2_5-VL-3B`**, pretrained on this image-based training set.
...
For live video captioning, we further fine-tune our `CASA-Qwen-2_5-VL-3B` models on the  
...
> 🔹 **LiveCC CASA models.**  
> We release **`CASA-Qwen-2_5-VL-3B-LiveCC`**, further finetuned on Live-WhisperX for live streaming.
...
We provide a `pyproject.toml` with the minimal dependencies required to run the model.
...
We provide a script to caption a video using our `CASA-Qwen-2_5-VL-3B-LiveCC` model and generate the resulting video with subtitles embedded at the actual time they are generated.
...
Using this pipeline, we evaluate our models `CASA-Helium1-VL-2B`, <span className="text-gray-400">`Helium1-VL-2B`</span>, and `CASA-Qwen-2_5-VL-3B`
...
<td>81.5</td><td>89.0</td><td>61.6</td>
<td>736</td><td>75.6</td>
<td>60.3</td><td>67.9</td><td>55.5</td><td>1732</td>
</tr>
<!---
<tr>
<td align="left"><span style="color:#60a5fa;">StreamChat 7B</span></td>
<td>◇</td><td>◇</td><td>◇</td>
<td>◇</td><td>72.4</td>
<td>61.7</td><td>76.6</td><td>62.4</td><td>1520</td>
</tr>
-->
<tr>
<td align="left"><span style="color:#fb923c;"><strong>CASA-Helium1-VL-2B</strong></span></td>
<td>73.6</td><td>83.9</td><td>48.5</td>
<td>730</td><td>71.1</td>
<td>59.0</td><td>63.2</td><td>54.6</td><td>1567</td>
</tr>
<tr>
...
(<code>Helium1-VL-2B</code> trained in the same conditions. CASA significantly outperforms current SoTA
...
<tr>
  <td align="left">
    <span style="color:#fb923c;"><strong>CASA-Qwen-2_5-VL-3B</strong></span>
  </td>
  <td>82.3</td><td>88.4</td><td>59.3</td>
  <td>794</td><td>76.6</td>
  <td>63.4</td><td>75.0</td><td>59.4</td><td>1932</td>
</tr>
</tbody>
</table>
...
<p align="center">
<em>
Results for <code>qwen2_5vl_casa</code>, adapted from frozen Qwen2.5-VL. CASA reaches performance close to the original
...
[casa-arxiv]: https://arxiv.org/abs/2410.00037
